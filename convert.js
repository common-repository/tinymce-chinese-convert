function convertText(textStr, convertLang2){
	...
	for ( var i = 0; i < textStr.length; i++ ){
		var thisWord = textStr.charAt(i);	// 取出一個字符.
			// charCodeAt()返回該中文的utf8編碼值, 2616個中文的utf8編碼值:繁 min=17079,max=40860, 簡 min=13726,max=40863
			// 故: 最小為13726, 最大為40863.
		var utf8Code = thisWord.charCodeAt(0);		// thisWord就只有一個字符.
		if ( utf8Code >= 13726 && utf8Code <= 40863 ){
			if( convertLang2.toLowerCase() == 'gb' ){	// 如該字的utf8編碼值沒有超出範圍, 嘗試看能否找出它在2616個字中的位置.
				var thisWordPosition = allTC.indexOf(thisWord);
			}else{
				var thisWordPosition = allSC.indexOf(thisWord);
			}
			if ( thisWordPosition != -1 ){	// 若此字在2616個字集中找得到的話, 轉換為對應值.
		...
		}else{		// 如該字的Unicod編碼值超出2616個中文的utf8編碼值範圍, 則不做轉換, 返回原字符.
			newStr += thisWord;
		}
	}
	textStr = null;
